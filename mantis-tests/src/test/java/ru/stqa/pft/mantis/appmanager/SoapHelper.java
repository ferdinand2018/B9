...
public class SoapHelper {
    ...
    public Issue addIssue(Issue issue) throws MalformedURLException, ServiceException, RemoteException {
        MantisConnectPortType mc = getMantisConnect();
        String[] categories = mc.mc_project_get_categories("soap.username", "soap.password", BigInteger.valueOf(issue.getProject().getId()));
    ...
    public Issue getIssue(int issueId) throws MalformedURLException, ServiceException, RemoteException {
        ...
        return new Issue().withId(myIssue.getId().intValue()).withSummary(myIssue.getSummary()).withDescription(myIssue.getDescription()).withStatus(myIssue.getStatus()).withProject(new Project().withId(myIssue.getProject().getId().intValue()).withName(myIssue.getProject().getName()));
    }
    ...
    public Set<Issue> getIssues(Project project) throws MalformedURLException, ServiceException, RemoteException {
        ...
        return Arrays.asList(issues).stream().map((i) -> new Issue().withId(i.getId().intValue()).withSummary(i.getSummary()).withDescription(i.getDescription()).withProject(project)).collect(Collectors.toSet());
    }
}
